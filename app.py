...
# --- Giả sử df đã load từ file hoặc có sẵn ---
...
if show_filters:
    ...
    selected_bar_levels = job_levels
...
for level in visible_levels:
    ...
    # Điều chỉnh width theo toggle filter
    base_width = 50 * len(ages) + 100
    if show_filters:
        chart_width = max(400, min(900, base_width))  # nhỏ hơn khi có filter
    else:
        chart_width = max(700, min(1200, base_width + 200))  # rộng hơn khi ẩn filter
    ...
    # Hiển thị 2 biểu đồ bên cạnh nhau
    if show_filters:
        col1, col2 = st.columns([1, 1])
    else:
        col1, col2 = st.columns([1.5, 1.5])  # rộng hơn khi ẩn filter
